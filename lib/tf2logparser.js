    ...
    var timestamp = parsingUtils.getTimestamp(logLine),
      ...
      if(parsingUtils.isLogLineOfType(logLineDetails, '"')) {
        ...
        if(playerLineAction == 'position_report') {
          _log.setPlayerPos(parsingUtils.getReportedPosition(logLineDetails));
          return;
          ...
          if(players[0].team == null && _log.getPlayerRoles(players[0].steamid).length == 0) {
            // It is safe to say the player is unimportant as they never joined a class and were not in a team (team includes all people who have fully joined the server).
            _log.removePlayer(players[0]);
          }
          return;
        } else if(playerLineAction == 'killed') {
          var attacker = players[0], victim = players[1], attackerCoord = parsingUtils.getKillCoords(logLineDetails, 'attacker'),
            victimCoord = parsingUtils.getKillCoords(logLineDetails, 'victim');
            
            if(!attacker || !victim || !attackerCoord, !victimCoord) return; //corrupt line
            
            attacker.position = attackerCoord;
            victim.position = victimCoord;
            
          _log.addKillEvent(timestamp, attacker, victim, parsingUtils.getWeapon(logLineDetails), parsingUtils.getCustomKill(logLineDetails));
          ...
          _log.addKillEvent(timestamp, players[0], players[0], parsingUtils.getWeapon(logLineDetails), 'suicide');
          ...
          if(trigger == 'kill assist') {
            ...
            _log.addExtinguishEvent(timestamp, players[0], players[1], parsingUtils.getWeapon(logLineDetails));
            return;
      ...
      } else if(parsingUtils.isLogLineOfType(logLineDetails, '[')) {
        // TODO: Are there any plugins we need output from?
        return; //ignore all other plugins
        ...
        || parsingUtils.isLogLineOfType(logLineDetails, 'server cvars ')
