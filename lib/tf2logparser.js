      ...
      var logLineDetails = parsingUtils.getLogLineDetails(logLine), players = null;
        ...
        if(parsingUtils.isLogLineOfType(logLineDetails, '"')) {
          ...
          if(playerLineAction == 'position_report') {
            ...
            _log.incrementWeaponStatToPlayer(attacker.steamid, weapon, 'kills');
            _log.incrementWeaponStatToPlayer(victim.steamid, weapon, 'deaths');
            _log.addWeapon(weapon);
            _log.incrementPlayerStatToPlayer(attacker.steamid, victim, 'kills');
            _log.incrementPlayerStatToPlayer(victim.steamid, attacker, 'deaths');
            return;
            ...
            _log.incrementWeaponStatToPlayer(players[0].steamid, weapon, 'deaths');
            _log.addWeapon(weapon);
            _log.incrementStatToPlayer(players[0].steamid, 'deaths');
            _log.incrementPlayerStatToPlayer(players[0].steamid, players[0], 'deaths');
