...
function Log() {
  ...
  */
  ...
  var _updateRoleSecondsPlayedToPlayer = function(foundPlayer, timestamp) {
    ...
    if(!_scratch.roleStart[foundPlayer.steamid]) _scratch.roleStart[foundPlayer.steamid] = _log.gameStartTimestamp;
  ...
  this.updateRoleSecondsPlayedForAllPlayers = function(timestamp) {
    for(var p in _log.players) {
      if(_log.players[p].online) _updateRoleSecondsPlayedToPlayer(_log.players[p], timestamp);
    }
  };
