...
function Log() {
  ...
  */
  ...
  /**
    Increments the given player stat for the player.
    ...
    @param playerStatPlayer full player object to record k/d by the player represented in playerSteamid
    @param statToIncrement property on object in players array to increment (ie. kills)
    @param increment Optional. If not specified, the given stat is incremented by 1. Otherwise, the value given here is added to the stat specified.
  */
  this.incrementPlayerStatToPlayer = function(playerSteamid, playerStatPlayer, statToIncrement, increment) {
    if(!playerSteamid || !playerStatPlayer || !statToIncrement) return;
    else if(statToIncrement != 'kills' && statToIncrement != 'deaths') throw new Error("Invalid statToIncrement given: "+statToIncrement);
    else if(increment === 0) return;
    ...
    if(foundPlayer.playerStats[playerStatPlayer.steamid] === undefined) {
      foundPlayer.playerStats[playerStatPlayer.steamid] = {
        name: playerStatPlayer.name, //only copying name and steamid to prevent having to constantly update other attributes that may not be necessary at this level.
        steamid: playerStatPlayer.steamid,
        kills: 0,
        deaths: 0
      };
    }
    
    foundPlayer.playerStats[playerStatPlayer.steamid][statToIncrement] += increment;
  };
