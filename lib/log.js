...
function Log() {
  ...
  */ 
  ...
  /**
    ...
    @param victim single player object retrieved from ParsingUtils#getPlayers.
  */ 
  this.addAssistEvent = function(timestamp, player, victim) {
    _log.events.push({timestamp: timestamp, type: 'assist', player: player, victim: victim});
  };
  ...
  */
  this.addRoleToPlayer = function(steamid, role) {
    ...
    if(foundRole === false) foundPlayer.roles.push({role: role});
  };
  
  /**
    Get an array of the roles a player has played
    @param steamid string representing the steamid of the player
  */
  this.getPlayerRoles = function(steamid) {
    if(!steamid) return;
    
    var foundPlayer = _getPlayer(steamid);
    if(foundPlayer === false) throw new Error("Could not find player: "+steamid);
    
    return foundPlayer.roles;
  };
