...
module.exports.playerSpread = function(players) {
  ...
  players.forEach(function(rowPlayer, rowIndex, rowArray) {
    var row = [
        rowPlayer.team
      , {name: rowPlayer.name, steamid: rowPlayer.steamid, friendid: rowPlayer.friendid}
    ];

    players.forEach(function(colPlayer, colIndex, colArray) {
      if(typeof colPlayer.playerSpread[rowPlayer.steamid] === 'undefined') row.push(0); //row player did not kill this column player
...
module.exports.itemSpread = function(players) {
  ...
  players.forEach(function(player, index, array) {
    ...
    items.forEach(function(item, itemIndex, itemArray) {
      if(typeof player.itemSpread[item] === 'undefined') row.push(0); //player did not pick up this item
